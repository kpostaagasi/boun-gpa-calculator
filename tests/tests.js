...
// BOUN GPA Calculator pure utilities ARE safe to import directly: boun-gpa-utils.js has no
...
// ============================================
// BOUN GPA Calculator — pure utility tests (imported from src/boun-gpa-utils.js)
// ============================================
test('BOUN GPA Calculator: parseHM converts HH:MM to minutes', () => {
    assertEqual(parseHM('00:00'), 0, 'midnight');
    assertEqual(parseHM('09:30'), 570, '09:30');
    assertEqual(parseHM('23:59'), 1439, 'end of day');
});

test('BOUN GPA Calculator: jsDayToMon maps Sunday-indexed to Monday-indexed', () => {
    assertEqual(jsDayToMon(1), 0, 'Mon -> 0');
    assertEqual(jsDayToMon(0), 6, 'Sun -> 6');
    assertEqual(jsDayToMon(6), 5, 'Sat -> 5');
});

test('BOUN GPA Calculator: blocksOverlap detects same-day time intersections only', () => {
...
test('BOUN GPA Calculator: scheduleHasOverlap scans all blocks across courses', () => {
    const clean = [
...
test('BOUN GPA Calculator: nextDeparture finds next time and wraps to tomorrow', () => {
...
test('BOUN GPA Calculator: urgencyTier thresholds (overdue/<2d danger, <=7d warning, else neutral)', () => {
...
test('BOUN GPA Calculator: countdownInfo computes signed days/hours and overdue flag', () => {
